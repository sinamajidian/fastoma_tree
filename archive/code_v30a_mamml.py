...
if __name__ == "__main__":
    ...
    # a global variable
    project_folder = "/work/FAC/FBM/DBC/cdessim2/default/smajidi1/fastoma/mammal/old/mml5/"
...
def filter_prot_mapped(query_species_names, query_prot_records_species,query_prot_names_species_mapped):
    ...
    for species_idx in range(len(query_species_names)):    
        ...
        if len(query_prot_names_species_i) != len(query_prot_records_species_i):
            print(query_species_name, "before filtering", len(query_prot_names_species_i), len(query_prot_records_species_i))
            ...
            for query_prot_name in query_prot_names_species_i:
                if query_prot_name in query_prot_ids_records:
                    ...
                    print("error",query_species_name, query_prot_name)
            print(query_species_name,"after filtering", len(query_prot_records_filterd))
...
keep_og_treshold_species_query =  93  #360
...
# In[ ]:
    ...
    threshold_least_query_sepecies_in_OG = 95 # 15
    kept_oma_species_num = 9 # 20
...
len(['S0005', 'S0002', 'S0009', 'S0001', 'S0004', 'S0007', 'S0006', 'S0008', 'S0003'])
...
number_max_workers = 10 # round(int(argv[3])*1.3)
...
if len(list_removed_species):
    print("These species are add from OMA database:",list_added_species)
...
num_eq_dash_b=0
c=0
for i in range(112866):
    if a_str[i]==b_str[i] and not a_str[i]=="-":
        num_eq +=1
        #print(a_str[i])
    if a_str[i]==b_str[i] and a_str[i]=="-":
        num_eq_dash +=1
    if a_str[i]=="-" and not b_str[i]=="-":
        num_eq_dash_a +=1
    if not a_str[i]=="-" and  b_str[i]=="-":
        num_eq_dash_b +=1
    if a_str[i]!=b_str[i] and not a_str[i]=="-" and not b_str[i]=="-":
        c+=1
    
print(num_eq,num_eq_dash)
...
# # Compare fastoma HOG/OG id  with output of oma standalone

# In[ ]:


oma_database_address = "/work/FAC/FBM/DBC/cdessim2/default/smajidi1/fastoma/mammal/mml_v4/"
oma_database_address+="omamer_v4_database/oma_path_/OmaServer.h5"
#oma_db = db.Database(oma_database_address+"omamer_v4_database/oma_path_/OmaServer.h5")
...
mammal_h5_oma = tables.open_file(oma_database_address)
#genomeTab = mammal_h5_oma.root.Taxonomy
#genome_df = pd.DataFrame(genomeTab.read())
genomeTab2 = mammal_h5_oma.root.Genome
scinames = genomeTab2.col('SciName')
print(scinames,"\n \n ")

#print(genomeTab2.col("EntryOff"))
#array([     0,  25706,  43819,  64419,  83987, 105349, 126574, 147767,167083], dtype=uint32)
# 25706  is offset of second of genome (entry table)
# 43819 is offset of human 

# tr|A0A075B6U7|A0A075B6U7_HUMAN T c
protein_entry=mammal_h5_oma.root.Protein.Entries
#print(protein_entry.description)
#protein_entry[43819],protein_entry[43820]

prt_name_omaSA  =[]
prt_HOGid_omaSA =[]
prt_OGid_omaSA  =[]

for prt_idx in range(43819,64419):
    prt_name_omaSA.append(protein_entry[prt_idx][10].decode('UTF-8'))
    prt_HOGid_omaSA.append(protein_entry[prt_idx][4].decode('UTF-8'))
    prt_OGid_omaSA.append(protein_entry[prt_idx][3])


print(prt_name_omaSA[:4],prt_HOGid_omaSA[:4],len(prt_name_omaSA))


# In[ ]:


protein_entry[64414:64419]


# In[ ]:


len(list(range(43819,64419)))
...
hog_OG_map_address_mml = "/work/FAC/FBM/DBC/cdessim2/default/smajidi1/fastoma/mammal/mml_v4/hog_og_map_mmlv4.dic"

# a global variable
project_folder =  "/work/FAC/FBM/DBC/cdessim2/default/smajidi1/fastoma/mammal/mml_v4/omamer_v4_search/"
#"/work/FAC/FBM/DBC/cdessim2/default/smajidi1/fastoma/test_github/test_data/" # argv[2]

(oma_db, hog_OG_map, list_speices) = parse_oma(oma_database_address, hog_OG_map_address_mml)


hog_list=hog_OG_map.keys()
print(len(hog_list))
...
equal_num=0
not_in=0
for i in range(len(prt_HOGid_omaSA)):
    hog = prt_HOGid_omaSA[i]
    if len(hog):
        og_from_omaSA = prt_OGid_omaSA[i]
        if hog in hog_OG_map:
            og_from_map = hog_OG_map[hog]

            if og_from_omaSA == og_from_map and og_from_omaSA!=0:
                #print(og_from_omaSA)
                equal_num+=1
            else:
                diff_num+=1
                a= (og_from_omaSA,og_from_map)
        else:
            not_in+=1 # why? 
        

print(a,equal_num,diff_num,not_in)
...
query_prot_names_human= query_prot_names_species[43]
#len("tr|A0A075B6U7|A0A075B6U7_HUMAN")>20


# In[ ]:


query_prot_names_human_edited=[]
for i in query_prot_names_human:
    
    if len(i)==20:
        query_prot_names_human_edited.append(i)
    elif len(i)>20:
        query_prot_names_human_edited.append(i[:20])
    elif len(i)==19:
        query_prot_names_human_edited.append(i+" ")
...
for idx in range(6319,20600):
    if not query_prot_names_human_edited[idx]==prt_name_omaSA[idx+1]:
        print(idx+1,query_prot_names_human[idx],prt_name_omaSA[idx+1])
...
for idx in range(5):
    print(idx+1,query_prot_names_human[idx])
...
# prot_list_omamer_edited=[]

# for i in query_prot_names_human:
    
#     if len(i) :
#         prot_list_omamer_edited.append(i[:21])
#     elif len(i)>10 and len(i)<21:
#         prot_list_omamer_edited.append(i+" ") # *(20-len(cc))
#         #print(i)
#     else:
#         print(i)
...
# vals=[]
# for i in prt_name_omaSA:
#     if i not in prot_list_omamer_edited:
#         vals.append(i)
...
# prot_list_omamer= query_prot_names_species[43]
# hog_list_omamer= query_hogids_species[43]

# print(len(prot_list_omamer),len(hog_list_omamer))

# match=0
# mismatch=0
# for idx in range(len(prot_list)):
#     prot= prot_list_omamer[idx]
#     hog=hog_list_omamer[idx]
...
# query_species_names[43] # .decode('UTF-8')
...
#[len(i) for i in prot_list_omamer_edited if len(i)<20]
...
#[i[:14] for i in prot_list_omamer_edited]
...
len("sp|P05155|IC1_HUMAN "),prot_list_omamer_edited[:10]
...
for i in range(10):
    print(prt_name_omaSA[i]==prot_list_omamer_edited[i])
